...
def analyze_genre_emotions(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    df_top_genres = (
        df_exploded[df_exploded["parsed_genres"].isin(top_genres)]
        .copy()
    ...
    summary_df = pd.concat([counts, rec_ratios, emotion_means], axis=1).sort_values(
        "件数", ascending=False
    )
    ...
    primary_dist = (
        pd.crosstab(
            df_top_genres["parsed_genres"],
            df_top_genres["primary_emotion"],
            normalize="index",
        )
        * 100
    ).round(2)
...
def main():
    ...
    genre_summary.to_csv(
        os.path.join(output_dir, "genre_emotion_summary.csv"), encoding="utf-8-sig"
    )
    genre_primary_dist.to_csv(
        os.path.join(output_dir, "genre_primary_dist.csv"), encoding="utf-8-sig"
    )
