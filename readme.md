...
また、Hugging Faceの感情分析モデルを用いて6つの基本感情軸 (喜び・怒り・嫌悪・悲しみ・恐怖・驚き) での感情分析および形態素解析による頻出語分析 (名詞・形容詞) を実施可能です。
...
│   └── insights/                       - Recommended比較・プレイ時間・ジャンル別感情分析・頻出語集計CSV
...
├── 05_analyze_genre_emotions.py        - Step 3-2: ジャンル別の感情抽出
├── 06_analyze_genre_playtime_correlations.py - Step 4-2: ジャンル別の相関分析
├── 07_analyze_frequent_words.py        - Step 5: 頻出語 (複合名詞および形容詞原形) の抽出・パーセンテージ算出
├── data_processor.py                   - 共通データ処理・ファイルI/O用モジュール
├── analysis_report.md                  - 各種分析結果・知見をまとめたレポート
└── readme.md
```

## 実行方法

各スクリプトを順番に実行することで、データの取得から分析までを一貫して行うことができます。
...
| **Step 3-2**| `uv run 05_analyze_genre_emotions.py` | ゲームジャンル別の感情プロファイルおよび評価構成の集計分析 |
| **Step 4** | `uv run 04_analyze_review_correlations.py` | Recommended / Not Recommended の感情比較およびプレイ時間別感情相関の定量分析 |
| **Step 4-2**| `uv run 06_analyze_genre_playtime_correlations.py` | 各ゲームジャンル内でのプレイ時間と感情割合の相関分析 |
| **Step 5** | `uv run 07_analyze_frequent_words.py` | 頻出語 (複合名詞および形容詞原形) の抽出とパーセンテージ算出 |
...
### バックグラウンド実行 (SSH切断・OOM対策)

長時間かかる処理を安全に完走させるため、`nohup` での実行を推奨します。
...
| ファイル名 | 説明 |
| --- | --- |
| [analysis_report.md](analysis_report.md) | 分析レポートメモ: 感情比較、プレイ時間相関、ジャンル別感情特性、頻出名詞・形容詞分析の総合レポート |
...
| `data/insights/frequent_nouns.csv` | 頻出名詞 (複合名詞) の抽出結果および出現件数・割合(%) |
| `data/insights/frequent_adjectives.csv` | 頻出形容詞 (基本形/原形) の抽出結果および出現件数・割合(%) |
